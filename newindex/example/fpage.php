...
$json = file_get_contents("http://www.babyinbook.com/babyinbook/interface.php/Index/GetBookInfo/id/{$_SESSION['bookid']}");
$res = json_decode($json,true);
$englishname = !empty($_REQUEST['englishname'])?$_REQUEST['englishname']:'0';
...
<!--		<input class="form-input" id="title" type="text" name="title" style="margin-left:20px;width:300px;color:#ccc;" value="--><?php //if($_POST['bookid']==59){echo 'Dear ' . $_POST['name'];}else{echo '亲爱的' . $_POST['name'];} ?><!--"/>-->
		<input class="form-input" id="title" type="text" name="title" style="margin-left:20px;width:300px;color:#ccc;" value="<?php if($_SESSION['bookid']==59){echo 'Dear ' . $_SESSION['name'];}else{echo '亲爱的' . $_SESSION['name'];} ?>"/>
		<div style="height:10px;"></div>
<!--		<div class="fo" style="margin-left:20px;border-bottom: 1px solid #ccc;width:90%;"><img style="width:10px;margin-top:8px;" src="http://www.babyinbook.com/babyinbook/images/line.png" />--><?php //if($_POST['bookid']==59){echo '写给宝贝的话  （限300字符内）' . $_POST['name'];}else{echo '写给宝贝的话  （限90字内）';} ?><!--</div>-->
		<div class="fo" style="margin-left:20px;border-bottom: 1px solid #ccc;width:90%;"><img style="width:10px;margin-top:8px;" src="http://www.babyinbook.com/babyinbook/images/line.png" /><?php if($_SESSION['bookid']==59){echo '写给宝贝的话  （限300字符内）' . $_SESSION['name'];}else{echo '写给宝贝的话  （限90字内）';} ?></div>
		...
		<input class="form-input" type="text" id="member" name="member" style="margin-left:20px;width:300px;color:#ccc;" value="<?php if($_SESSION['bookid']==59){echo 'Mom and Dad';}else{echo '爱你的爸爸妈妈';}?>"/>
		<div style="height:10px;"></div>
		<?php 
			$disabled_arr = array(18,64,65,66,61);
		?>
		<?php if(!in_array($_SESSION['bookid'],$disabled_arr)){?>
		<div class="fo" style="margin-left:20px;border-bottom: 1px solid #ccc;width:90%;display:none;"><img style="width:10px;margin-top:8px;" src="http://www.babyinbook.com/babyinbook/images/line.png" />日期</div>
		<input class="form-input" type="date" value="<?php echo date("Y-m-d");?>" name="date" style="margin-left:20px;width:300px;display:none;"/>
		<?php } ?>
		<div style="height:10px;"></div>
	...
	function checkform(){
		...
		var bookid=<?php echo $_SESSION['bookid'];?>;
		if (bookid==59) {
			if(title_len.length > 25){
				alert("被寄语人名字长度不可超过20字符！");
				return false;
			}
			
			if(text_len.length > 300){
				alert("寄语不可超过300字符！");
				return false;
			}
