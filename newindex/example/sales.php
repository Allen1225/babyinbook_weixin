...
if(!empty($_SESSION['openId'])){
	...
	$sql = "SELECT
	sum(commission_fee) as sum,count(commission_fee) as count,substr(payment_date,1,7) as date
FROM
	babyinbook.commission
WHERE
	channel_id = '$channelid'
GROUP BY substr(payment_date,1,7)
order by substr(payment_date,1,7) desc";
$res = mysql_query($sql);
	while($a = mysql_fetch_assoc($res)){
		$commission[] = $a;
	}
...
<body>
<div class="content">
<?php $i=0; foreach($commission as $key=>$value){ 
if($i==0){ $i++;
?>
<a href="showsalesdetail.php?time=<?php echo $value['date']; ?>">
...
订单数: <?php echo $value['count']; ?>，提成：<?php echo $value['sum']; ?>
</div>
<div style="color:white;font-size:40px;width:40px;height:40px;position:absolute;right:10px;top:55px;">></div>
</div></a>
<?php }else{ ?>
<a href="showsalesdetail.php?time=<?php echo $value['date']; ?>"><div style="width:100%;height:55px;border-bottom:1px solid rgb(198,198,198);position:relative;">
<div style="font-size:15px;width:260px;height:40px;position:absolute;left:20px;top:5px;">
<font style="font-weight:bold;"><?php echo $value['date']; ?></font><br/>
订单数: <?php echo $value['count']; ?>，提成：<?php echo $value['sum']; ?>
</div>
<div style="line-height:40px;font-size:40px;width:40px;height:40px;position:absolute;right:10px;top:5px;">></div>
</div></a>
<?php }} ?>
</div>
